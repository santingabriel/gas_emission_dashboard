...
#---- executar sempre que sair do servidor .\venv\Scripts/activate (sempre em que no terminal não estiver o "venv" no início) e streamlit run Dashboard.py e Set-ExecutionPolicy Unrestricted
# ----- Page configs--
...
with st.sidebar.expander('Gás'):
    ...
    f_gases = st.multiselect('Selecione os gases', gases, default=gases)
...
emissoes_estados = dados.drop_duplicates(subset='Estado')[['Estado', 'lat', 'long']].merge(emissoes_estados, on='Estado').sort_values(by='Emissão', ascending=False).reset_index()
emissoes_estados.drop('index', axis=1, inplace=True)

emissoes_estado_gas = dados.groupby(['Estado', 'Gás'])[['Emissão']].sum().sort_values(by='Emissão', ascending=False).reset_index()
...
with tab_gas:
    ...
    with st.container(height=500):
        ...
        for gas in emissoes_gas_ano.columns:
            ...
            fig.update_layout(yaxis_title='ERmissão', showlegend=False)
            st.plotly_chart(fig)
